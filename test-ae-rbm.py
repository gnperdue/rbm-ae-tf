...
for i in range(FLAGS.epochs):
    ...
    print(
        rbmobject3.compute_cost(
            rbmobject2.transform(rbmobject1.transform(trX))
        )
    )
...
for i in range(FLAGS.epochs):
    ...
    print(
        rbmobject4.compute_cost(
            rbmobject3.transform(
                rbmobject2.transform(rbmobject1.transform(trX))
            )
        )
    )
...
plt.scatter(
    autoencoder.transform(teX)[:, 0], autoencoder.transform(teX)[:, 1],
    alpha=0.5
)
plt.show()
